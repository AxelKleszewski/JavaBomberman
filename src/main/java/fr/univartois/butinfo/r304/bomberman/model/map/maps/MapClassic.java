...
public class MapClassic implements IMap {

    /**
     * Chance to place a brick on the lawn.
     */
    public static final double CHANCE_TO_BE_BRICK = 0.3;

    /**
     * Initializes the given GameMap with the required map elements.
     * The map layer here is the classic one.
     *
    ...
    public void initializeMap(GameMap gameMap) {
        ...
        for (int i = 2; i < height - 2; i += 4) {
            for (int j = 2; j < width - 2; j += 4) {
                gameMap.setAt(i, j, new Cell(new Wall(new UnbreakableWall(wall))));
            }
        }
